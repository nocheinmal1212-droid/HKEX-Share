...
def evaluate_batch(batch_path, authored=False):
    ...
    assert batch['version'] == identity['batch'] and batch['runtime_code_sha256'] == identity['runtime_identity']
    assert batch['configurations'] == read(PREP/'configurations.json')
    ...
    for ex in expected['queries']:
        ...
        try:
            consumption = read(batch_path/'operational'/name/'consumption.json')
            assert consumption['model_calls'] == 0
            ...
            assert consumption['outcome'] == prov['outcome']
            if ann:
                assert roles['primary']['host_state'] in ('accepted','abstained')
            ...
            persisted = {'available':False,'reason':'missing_partial_or_invalid_consumption'}
        results.append({'complete_primary_success':roles['primary']['success'] and persisted['available'], 'name':name,'roles':roles,'primary_persistence':persisted})
    return {'version':'confirmation-evaluator-1.1.0','authored_diagnostic':authored,
            ...
            'complete_primary_successes':sum(x['complete_primary_success'] for x in results),
