...
LIVE = ROOT / 'runs/semantic-citation-confirmation-2026-09-19/live-v2'
...
def main():
    ...
    prior = read(ROOT/'eval/semantic-citation-confirmation/2026-09-19/preflight-v1/execution-identity.json')
    assert semantic.VERSION == prior['operation']
    assert sha(semantic.PROMPT.encode()) == prior['prompt_sha256']
    assert fingerprint(semantic.SCHEMA) == prior['schema_sha256']
    assert header_support.policy_identity() == prior['policy']
    assert semantic_attempt.CONFIGS == read(ROOT/'runs/semantic-citation-confirmation-2026-09-19/prepared-v1/configurations.json')
    selection = semantic_cli.normalized_selection(ROOT/'runs/semantic-citation-confirmation-2026-09-19/prepared-v1/selection.json')
    retained = read(ROOT/'runs/semantic-citation-confirmation-2026-09-19/prepared-v1/queries.json')
