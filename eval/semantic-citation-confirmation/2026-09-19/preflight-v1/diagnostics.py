...
OUT = ROOT/'runs/semantic-citation-confirmation-2026-09-19/diagnostics-v1'
...
def metadata(role,mode):
    ...
    return {'transport_status':'ok','body_base64':base64.b64encode(encode({'data':data})).decode()}
...
def budget_batch(mode,real_consumer=False):
    ...
    def isolated(function,payload):
        if function == 'prepare_worker':
            if mode == 'total_time': clock[0] += 170
            return PREPARED
        if real_consumer: return original_isolated(function,payload)
        if mode == 'total_time': clock[0] += 170
    ...
    def transport(job):
        ...
        if job['mode'] == 'metadata':
            if mode == 'total_time': clock[0] += 55
            if mode == 'admission_time': clock[0] += 550
    ...
    if mode == 'total_time':
        result['hard_600_second_ceiling_satisfied'] = clock[0] <= 600
        result['expected_boundary'] = 'Hard total <=600, including preparation and consumption'
        result['actual'] = 'Dispatcher returns normally after virtual 730s; its own elapsed excludes 170s preparation'
        assert clock[0]==730 and summary['elapsed_seconds']==560
    ...
    for variant in ('research_missing','research_corrupt','primary_corrupt'):
        ...
        for name in EXPECT:
            ...
            records.append({'variant':variant,'name':name,'primary':read(out/'provenance.json')['outcome'],
                            'research':r['result'].get('validation',{}).get('state',r['result']['state']),
...
def blank_control():
    specs = read(ROOT/'runs/paired-semantic-integration-2026-09-18/queries.json')
    spec = next(s for s in specs if s['name']=='missing_target')
    q = project(read(SELECTION['evidence']['path']),spec)
    batch = OUT/'blank-target-control'
    assert q['gate']=='missing_evidence'
    for role in CONFIGS:
        d=batch/role/spec['name'];d.mkdir(parents=True)
        def forbidden(_): raise AssertionError('local control called transport')
        r=dispatch(q,request(q,role,batch.name),forbidden,Store(d))
        assert r['model_calls']==0 and r['validation']['state']=='missing_evidence'
    out=batch/'operational'/spec['name'];out.mkdir(parents=True)
    cli.isolated('consume_worker',{'selection':SELECTION,'spec':spec,'batch':str(batch),'output':str(out),'nonce':batch.name})
    assert read(out/'annotations.json')==[]
    put(OUT/'blank-control.json',{'gate':q['gate'],'model_calls':0,'annotations':0,'live_case':False})
...
def main():
    ...
    for mode in ('route_missing','metadata_identity','over_price','metadata_timeout','cost_cap','http_cap','role_cap','admission_time','total_time'):
        budget_batch(mode)
    case_matrix()
    role_isolation()
    blank_control()
    assert not Path(read(PACKET/'launch-proposal.json')['live_output']).exists()
    print('All authored diagnostics complete; total-time authorization blocker retained.',flush=True)
