...
from hkex_audit.artifacts import encode, fingerprint, read, require, sha, write_new, loads
...
def review_bytes(path):
    """Read an explicitly selected active input after enforcing its path boundary."""
    path = no_symlinks(path)
    require(path.suffix.lower() in {'.json', '.jsonl', '.md', '.py'},
            'review input type forbidden: no PDFs, images or PDF text exports')
    require(path.name.lower() not in {'error_detail.jsonl', 'apikey.txt'}
            and not any(p.lower().startswith('.env') for p in path.parts),
            'forbidden review input')
    require(path.is_file(), 'review input is not a regular file')
    return path.read_bytes()


def review_read(path):
    return loads(review_bytes(path))
...
def verify(directory, expected=None, root=None):
    ...
    for item in review['inputs']:
        ...
        if root is not None:
            ...
            require(sha(review_bytes(path)) == item['sha256'], 'source input changed: ' + item['path'])
