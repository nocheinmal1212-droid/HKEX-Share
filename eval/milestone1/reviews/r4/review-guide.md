...
Use [index.html](index.html) to review each item and [review.json](review.json) for its traces.
Record the `approval_fingerprint` in [packet-manifest.json](packet-manifest.json) with your reviewer
...
Owner acceptance remains pending and headline status remains UNGATED.
