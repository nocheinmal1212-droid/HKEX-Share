...
def review_bytes(path):
    """Restrict active review inputs before opening them; no PDF or image/text exports."""
    path = Path(path)
    require(not any(p.is_symlink() for p in [path, *path.parents]),
            'review symlink input or parent is forbidden')
    require(path.suffix.lower() in {'.json', '.jsonl', '.md', '.py'},
            'review input type forbidden: no PDFs, images or PDF text exports')
    require(path.name not in {'error_detail.jsonl', 'APIKEY.txt'}, 'forbidden review input')
...
def prepare(runs, output):
    ...
    for variant, entry in verification['variants'].items():
        ...
        selected_files(selection)  # Apply runtime PDF/evaluation/symlink exclusions before native reads.
    ...
    capture(ROOT/'eval/milestone1/native-review-guide.md', 'review-guide.md')
    ...
    packet = {'schema_version': '2.0.0', 'mode': 'fixture', 'review_status': 'pending',
              ...
              'additional_evidence': regions, 'fixtures': fixtures,
              'guidance': guidance, 'evidence_authority': 'selected_native_output', 'ocr_quality': 'out_of_scope', 'publication_status': 'UNGATED', 'headline': None,
    ...
    html += '<p class="banner">' + escape(guidance['source_of_truth']) + '</p>'
    html += '<nav>Review: ' + ' · '.join('<a href="#'+c['error_id']+'">'+c['error_id']+'</a>' for c in cfg['cases'])
    html += ' · <a href="#region-123-0">Partial table</a> · ' + ' · '.join('<a href="#fixture-'+f['fixture_id']+'">'+f['fixture_id']+'</a>' for f in fixtures) + '</nav>'
