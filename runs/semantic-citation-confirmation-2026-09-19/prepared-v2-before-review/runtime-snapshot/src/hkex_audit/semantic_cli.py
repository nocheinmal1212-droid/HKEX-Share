...
def send_transport(job, prefix, deadline):
    ...
    if time.monotonic() >= deadline.work_end:
        ...
        record = transport({**job, 'deadline_monotonic':deadline.work_end}, timeout=deadline.remaining(60))
...
def run(args):
    ...
    write_new(output/'launch.json', {'version': BATCH_VERSION, 'started_monotonic': started,
    ...
    write_new(output/'supervisor.json', {**result, 'state':'finished' if success else 'interrupted_or_failed',
