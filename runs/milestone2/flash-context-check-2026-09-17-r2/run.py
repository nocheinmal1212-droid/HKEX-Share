...
prep.RUN=ev.RUN=RUN
prep.CODES=prep.CODES+['model_selection_worker_v3.py','model_selection_worker_v3_entry.py']
...
def initialize():
    ...
    prep.batch('flash-r2-core',[{'model':k,'case':c} for k,c in [('v4flash','S08'),('v41flash','S08'),('v41flash','S11'),('v4flash','S11')]])
...
def launch(name,preflight_only=False):
    ...
    for preflight in ([True] if preflight_only else [True,False]):
        ...
        cmd=['/usr/bin/sandbox-exec','-f',str(stage/'sandbox.sb'),str(Path(sys.executable).resolve()),'-I','-B',str(stage/'model_selection_worker_v3_entry.py'),'--manifest',str(stage/'manifest.json'),'--out',str(RUN/name/'attempts')]
...
def continuation(core):
    ...
    prep.batch('flash-r2-confirmation',specs);launch('flash-r2-confirmation')
    return evaluate('flash-r2-confirmation')
...
def explanations(core,follow):
    ...
    if not specs:return []
    prep.batch('flash-r2-diagnostics',specs);launch('flash-r2-diagnostics');return evaluate('flash-r2-diagnostics')
...
if __name__=='__main__':
    ...
    if a.mode=='prepare':initialize()
        ...
        for n in ['flash-r2-core','flash-r2-confirmation','flash-r2-diagnostics']:
        ...
        launch('flash-r2-core');core=evaluate('flash-r2-core')
